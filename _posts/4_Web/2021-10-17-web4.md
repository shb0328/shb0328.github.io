...
Entity 클래스란 실제 DB와 매칭될 클래스이다.
데이터 수정이 필요할 경우, DB에 실제 쿼리를 날리지 않고, Entity 클래스의 수정을 통해 작업할 수 있다.

JPA에서 제공하는 `@Entity` 어노테이션을 이용해서 Entity 클래스를 선언해줄 수 있다.
...
자바빈 규약에 따라 Getter/Setter를 무작정 생성하는 경우가 있다.
...
주문 취소 이벤트가 발생하면 `setStatus`를 통해서 `false`값을 넘기는 것이 무엇을 의미하는지 코드상에 명확히 드러나있지 않다.
...
하지만 이 방법은 채워야할 필드가 무엇인지 명확하지 않아 디버깅이 어렵다는 단점이 있다.
...
반면에, `Builder`를 사용하면 어느 필드에 어떤 값을 채워야할지 명확하게 인지할 수 있다.
